...
$Params = @{
    ...
    Settings           = @{fileUris = @('https://raw.githubusercontent.com/elfredburger/azure_task_13_vm_monitoring/main/install-app.sh'); commandToExecute = './install-app.sh' }
}
Set-AzVMExtension @Params

# Install Azure Monitor Agent VM extention ->
Set-AzVMExtension -Name AzureMonitorLinuxAgent -ExtensionType AzureMonitorLinuxAgent -Publisher Microsoft.Azure.Monitor -ResourceGroupName $resourceGroupName -VMName $vmName -Location $location -TypeHandlerVersion "1.0" -EnableAutomaticUpgrade $true
